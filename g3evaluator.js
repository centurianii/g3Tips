...
g3.evaluator = (function(){   
   ...
   function init(){
      ...
      /*
       * add/remove optional library button actions:
       * label's text will become the script's id and
       * input's value will become the script's path
       */
      $("form#optional-addRemoveLib legend").click(function(){
         $("form#optional-addRemoveLib div").slideToggle();
      });
      $("form#optional-addRemoveLib button, form#optional-addRemoveLib input[type='button']").click(function(event){
         if($(this).val() === 'Add'){
            var found = false;
            if(nodes.$opt_name.val()){
               $('form#optional-libraries label').each(function(){
                  if($(this).text() === nodes.$opt_name.val()){
                     found = true;
                     return false;
                  }
               });
               if(!found){
                  var id = g3.utils.randomString(5);
                  var txt = '<label class="label" for="' + id + '">' +
                            nodes.$opt_name.val() +'</label><input type="checkbox" id="' +
                            id + '" value="' + nodes.$opt_path.val() + '" /><br />';
                  $("#optional-libraries fieldset").append(txt);
               }
            }
         }else if($(this).val() === 'Remove'){
            var found = -1;
            if(nodes.$opt_name.val()){
               $('form#optional-libraries label').each(function(ndx){
                  if($(this).text() === nodes.$opt_name.val()){
                     found = ndx;
                     return false;
                  }
               ...
               if(found > -1){
                  $("form#optional-libraries label").eq(found).next('input').addBack().next('br').addBack().remove();
               }
            }
         }
      });
      ...
      /*
       * load all libraries check box behaviour
       */
      $("#loadall").prop('checked', false);
      $("#loadall").on('change', function(event){
         var delay = 0;
         $('form#libraries input[type="checkbox"]').each(function(){
            var $node = $(this);
      ...
      /*
       * optional library check box behaviours
       */
      $("#optional-libraries input").prop('checked', false);
      $("#optional-libraries").on('change', 'input', function(event){
         $("#optional-libraries input").each(function(){
            //var $script = $('script[id=\'' + $(this).val() + '\']');
            //label's text is the script's id
            var $script = $(document.getElementById($(this).prev().text()));
            if($script.length === 0 && $(this).prop("checked")){
